...
describe "Traject::SolrJsonWriter" do
  ...
  describe "commit" do
    it "commits on close when set" do
      ...
      last_solr_post = @fake_http_client.post_args.last

      assert_equal "http://example.com/update/json?commit=true", last_solr_post[0]
    end
    ...
    it "commits on close with commit_solr_update_args" do
      ...
      assert_equal "http://example.com/update/json?softCommit=true", last_solr_post[0]
    end
    ...
    it "can manually send commit with specified args" do
      ...
      last_solr_post = @fake_http_client.post_args.last
      assert_equal "http://example.com/update/json?commit=true&optimize=true&waitFlush=false", last_solr_post[0]
    end
