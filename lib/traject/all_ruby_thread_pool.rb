...
module Traject
  # EXPERIMENTAL: This is a semantic copy of standard Traject::ThreadPool, but where
  # standard uses java.util.concurrent directly, here we use ruby_concurrent. 
  # This will allow it to run on MRI too. Theoretically we could replace Traject::ThreadPool
  # with this one, but for now it's an experiment to be used in classes we insist on
  # being MRI compatible. 
  #   * Is it a problem using unstable pre-1.0 ruby-concurrent?
  #   * Will it hurt performance on jruby? Shoudln't, but does it? We're using
  #     a ruby Queue for exceptions, which lacks Java's #poll, so is more
  #     expensive to check, which we do all the time. 
  #
